...
import { Activity, ChevronDown, CircleHelp, Menu, PanelLeft, Settings2, WifiOff, X } from "lucide-react";
...
export const Route = createFileRoute("/")({
    head: () => ({
        meta: [
            { title: "Gemma Local AI Side Panel" },
            { name: "description", content: "Private, local-first chat for Ollama and Gemma models." },
            { property: "og:title", content: "Gemma Local AI Side Panel" },
            { property: "og:description", content: "Private, local-first chat for Ollama and Gemma models." },
                    ...
                    <div className="flex h-[68px] items-center justify-between border-b border-sidebar-border px-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div className="tag-mono text-muted-foreground">local side panel</div>
                            </div>
                        </div>
                        <Button
                            variant="ghost"
                    ...
                    <div className="border-t border-sidebar-border p-4">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="flex items-center gap-1.5 text-xs">
                                <span className={cn("size-1.5 rounded-full", statusReady ? "bg-success" : "bg-warning")} />
            ...
            <main className="flex min-w-0 flex-1 flex-col">
                <header className="flex min-h-[68px] items-center justify-between border-b border-border bg-surface/70 px-4 backdrop-blur sm:px-6">
                    <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <div className="truncate text-sm font-semibold">
                                {view === "settings" ? "Settings" : activeConversation?.title ?? "New conversation"}
                            </div>
                            <div className="tag-mono mt-0.5 flex items-center gap-2 text-muted-foreground">
                                {statusReady ? (
                                    ...
                                    <>
                                        <WifiOff className="size-3 text-warning" /> local runtime needs attention
                                    </>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="flex items-center gap-2">
                        {view === "chat" && ai.models.length > 0 ? (
                            <label className="relative hidden items-center sm:flex">
                                ...
                                <select
                                    ...
                                    onChange={(e) => ai.patchSettings({ selectedModel: e.target.value })}
                        ...
                        <Button variant="ghost" size="icon" aria-label="Help and architecture" title="Architecture notes">
                        ...
                        <div className="mx-auto max-w-3xl">
                            <div className="mb-5 flex items-end justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-muted-foreground">Stored locally in IndexedDB.</p>
                                </div>
                                <span className="tag-mono text-muted-foreground">{ai.conversations.length} threads</span>
                            </div>
                                ...
                                {ai.conversations.map((conversation) => (
                                    ...
                                    >
                                        <span className="min-w-0">
                                            ...
                                            <span className="tag-mono mt-1 block text-muted-foreground">{conversation.model}</span>
                                        </span>
